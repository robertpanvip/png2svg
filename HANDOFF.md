...
## §11.12 A 方案实施与定案（2026-09-11 凌晨-白天，用户拍板"先试 A"）

**Generator 减压（`dataset/generator.py`）**：min_bbox 0.12→0.18、max_bbox 0.55→0.60、overlap 0.7→0.45、alpha 0.35→0.25、stroke 0.35→0.30、gradient 0.5→0.35、line 0.15→0.08/cubic 0.28、closed 概率 quad 0.8/cubic 0.85/arc 0.7。

**Go/No-Go 门控（通过）**：solid bbox 面积中位 0.087→0.135、closed 占比 84%；erode40 池化 corr 0.42→0.45-0.51（L1 0.22→0.178）、oracle 掩码 corr 0.65（L1 0.131）。

**从零 40k（runs/genA40k）**：11k 步 NaN 崩溃（nseg NaN），10k ckpt 保留——mae 0.067、bbox L1 0.157，**预测覆盖掩码 max 中位 1.0（旧分布 40k 仅 0.22）**——新分布下形状收敛快得多。但颜色头依旧全员灰。

**A 定案：分布减压是必要非充分**。CPU head-only 在线实验（新分布+新特征）：app 0.35-1.05 震荡不收敛；固定场景仍能学到 0.29。结合 §11.11 全部证伪链，剩余唯一未堵死的病根解释：**回归损失下"均值灰"是理性解**（池化收益 L1 0.178 vs 灰 0.26 太小，联合训练中被几何/渲染梯度淹没）。

## §11.13 色板分类方案（进行中）：监督范式改变，非容量堆叠

**依据**：回归→均值平衡点病理的正面解法。48 色板（`model/palette.py`，12 hue×2s×2v），solid fill 从色板采样；`palette_head`（Linear 324→48，仅 +3.3k 参数）分类 CE（w-palette=1.0），softmax 加权色**直接写入 slot 契约 rgb 字段**（渲染/解码自动一致，推理零额外开销）。

**两个已修的坑**：① palette_head 只接 h 时 CE 卡 ln(48) 随机——h 无颜色信息（§11.11 定案），必须接 color_in(h+hrf+cf_raw)；② NaN 防护必须在 `slots_to_objs` 之前（loss 处检查太晚），train.py 已前移。

**当前**：genA_pal = genA40k/10k 基线 + 色板 CE 微调 20k（lr 1e-4），pal CE 3.87(随机)→3.6-3.7 平台缓慢下降，斜率判定点 20k。验收判据 = matched_fill_check 颜色脱离灰均值 + 色板 top-1 精度。

## §11.14 advice.md 评估（2026-09-11，用户提交的外部诊断）

逐条核对（对照本 HANDOFF 已完成实验）：
- ✅ **"信息提取难于预测"（§4）**：与 §11.11 信息上限实测完全一致（bbox 0.27/erode 0.42/GT mask 0.65）。
- ✅ **Experiment B/C（§5）**：即 §11.12 已完成的 A 方案+oracle 探针，结论比 advice 预设更深——分布减压后颜色仍不收敛，另有回归均值病理。
- 🟢 **采纳：Oracle Benchmark 指标体系化（§6-7）**、**难度 profile（§8，即原路线图 #27 课程训练）**、**单变量对照纪律（§9）**——palette 微调即"只改监督范式一个变量"，对照基线 genA40k。
- ⚠️ **"不要继续堆颜色网络"（§3/§10）**：同意不堆容量；palette 方案不是容量堆叠（+3.3k 参数）而是监督范式改变，且不违背"先跑完 baseline 再单变量实验"流程（40k 从零训练期间架构未动）。
- ✅ 不扩参数/不重做 Transformer/暂缓 Rust-WASM：与现有判断一致。

## §11.15 色板方案判定（2026-09-11 晚，20k 步判定点）：证伪，颜色问题最终定位

genA_pal 20k 步判定（`benchmarks/palette_acc.py`）：
- pal CE 3.87(随机)→3.51-3.78 震荡平台，无持续斜率
- **色板 top-1 命中 0.108 < 随机 0.167；hue 档命中 0.248 << 随机 0.667**——分类头 mode collapse 到少数高频类
- matched 颜色：pred 从灰 0.37 移到 0.60 灰（高频色板类），与 GT 依旧无关

**结论升级（因果链闭合）**：连无均值退路的 CE 分类都学不动 → 瓶颈不在监督范式/损失权重/优化，而在 **cf_raw 级特征（corr~0.5 的"均值层面"弱相关，被描边+遮挡污染）不足以支撑颜色判别**。"填充区取色"= 分割级子任务（哪些像素是本对象填充、排除描边/遮挡），GT 掩码 corr 0.65 是天花板但模型推不出该掩码。

**剩余对症路线（需用户拍板）**：
- **E（对症）：填充掩码辅助任务**——dense 监督预测"对象内部填充掩码"（GT 免费可生成），分割学会后再在其上取色。真正把 object-centric inference 变成显式子任务。工作量：渲染器出 GT 掩码 + 一个轻量解码头 + BCE/ Dice 损失。
- **D（止损）：颜色接受吸收**，优先 P3b dash / P4 effects（可微 blur/shadow），颜色作为长期课题挂起。当前几何/结构管线已可用（匹配 100%、形状准），部署价值不受阻。

## §11.16.3 E 路线终审：泛化证伪（2026-09-12 凌晨，probe_cfmask 两次判定）

**实验设置**：E 路线 = 填充掩码辅助取色（cf_mask dense 分割头，w-mask 3.0 + w-fill 2.0）。genE 22k → genE2 续训至 40k（runs/genE2，sub_px=1 + no-ckpt）。判定工具 `benchmarks/probe_cfmask.py`（16 场景 / 38 solid 对）。

**两次判定数据**：

| 指标 | 24k 中途 | 28k 终审 | 灰基线 | 判定线 |
|---|---|---|---|---|
| cf_mask L1 | 0.404 | **0.404** | — | ≤0.34（改善≥15%） |
| pred L1 | 0.220 | 0.219 | 0.227 | ≤0.20（破灰） |
| cf_raw L1 | — | 0.280 | — | — |
| cf_mask 胜灰比例 | — | 6/38 | — | — |

cf_mask L1 两次**完全持平 0.404**，4000 步（约 35 分钟 GPU 训练）零改善；pred L1 0.219 ≈ 灰 0.227，未破灰；cf_mask 对象级胜灰仅 6/38。辅助掩码头自身在线泛化不动（离线 GT-IoU 0.62 可学 ≠ 在线可泛化），取色收益为零。
